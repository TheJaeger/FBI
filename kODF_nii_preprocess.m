function kODF_nii_preprocess(FT_parameters)
...
%     temp=dir('*_FAx.nii');
    temp=dir('fa.nii'); % HM added 10/10/18
    [pth,nam,ext]=fileparts(temp.name);
    hdr=spm_vol(fullfile(subj_dir,[subj_name temp.name])); % header template   
    ...
    if input_struc.odf_optimization
        ...
        DT = spm_read_vols(spm_vol('DT_masked.nii'));
        KT = spm_read_vols(spm_vol('KT_masked.nii'));
        
        DT = reshape(DT,[prod(hdr.dim),6])';
        KT = reshape(KT,[prod(hdr.dim),15])';
        
%         DT=spm_read_vols(spm_vol(fullfile(subj_dir,[subj_name temp.name(1:strfind(nam,'_')-1) '_DT.nii'])));    %Diffusion tensor (DT)
%         KT=spm_read_vols(spm_vol(fullfile(subj_dir,[subj_name temp.name(1:strfind(nam,'_')-1) '_DK.nii'])));     %Kurtosis tensor (KT)
%         load(fullfile(subj_dir,'DT.mat'));
%         load(fullfile(subj_dir,'KT.mat'));
%         KT = zeros(15,size(DT,2));
%         KT=reshape(KT,[prod(hdr.dim),15])';
%         DT=reshape(DT,[prod(hdr.dim),6])';
...
if input_struc.odf_optimization
    ...
    %Initialize output parameters
        ...
        input_struc.degree = 6;
        
        B_fbi=getSH(input_struc.degree,[ atan2(R(:,2),R(:,1)) acos(R(:,3))],'real');%EM
        B_fbi=B_fbi(:,cell2mat(Harm_id(1:input_struc.degree/2+1)));%EM
...
%                         [p,fval,ef] = fminunc(fK,DKI_V(j,:),BFGS_options);
                ...
                A2l(:,i)=(B_fbi'*B_fbi)^-1*B_fbi'*DKI_P/DKI_max(1) *fa_n(i); % scale the odfs based on fa so I can control stopping criteria in mrtrix 
    ...
    if input_struc.make_fib_file ==1 
        ...
        C = permute(reshape(odf_d,dimension),permute_img); 
    ...
    if input_struc.wrt_flg    
        ...
        for l=0:2:input_struc.degree
           for m=2:2:2*l+1
            SH_coeff(:,:,:,m+count) = SH_coeff(:,:,:,m+(2*(l-2)+1))*-1; % mrtrix does not have (-1)^m in formulas!
           end
           count=count+2*l+1;
        end
